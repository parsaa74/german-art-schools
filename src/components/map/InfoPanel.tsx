...
export default function InfoPanel({ school, isOpen, onClose }: InfoPanelProps) {
  const panelRef = useRef<HTMLDivElement>(null)

  // Log the props for debugging
  console.log('InfoPanel rendering with:', {
    isOpen,
    school: school?.name,
  ...
  // Log when the component renders and ensure visibility
  useEffect(() => {
    console.log('InfoPanel useEffect triggered with:', {
      isOpen,
      school: school?.name,
      schoolType: school?.type
    })

    // Ensure the panel is visible when it should be
    if (panelRef.current) {
      if (isOpen && school) {
        console.log('InfoPanel: Making panel visible for school:', school.name)
        panelRef.current.style.display = 'block'
        panelRef.current.style.opacity = '1'
        // Add a highlight animation
        panelRef.current.style.animation = 'pulse-glow 2s ease-in-out'
      } else {
        console.log('InfoPanel: Hiding panel')
    ...
    // Add a CSS animation for the glow effect
    const style = document.createElement('style')
    style.textContent = `
      @keyframes pulse-glow {
        0% { box-shadow: 0 0 20px rgba(41,121,255,0.4), 0 0 10px rgba(41,121,255,0.2) inset; }
        50% { box-shadow: 0 0 40px rgba(41,121,255,0.7), 0 0 20px rgba(41,121,255,0.5) inset; }
        100% { box-shadow: 0 0 20px rgba(41,121,255,0.4), 0 0 10px rgba(41,121,255,0.2) inset; }
      }
    `
  ...
  // For debugging purposes, log when the component renders
  useEffect(() => {
    console.log('InfoPanel rendered with school:', school?.name)
  }, [])

  // If no school, render nothing
  if (!school) {
    console.log('InfoPanel returning null because school is null')
    return null
  }

  console.log('InfoPanel rendering with school:', school.name, 'isOpen:', isOpen)

  // We already have a key press handler for the Escape key above
  ...
  return (
    <div
      ...
      aria-modal="true"
      className="fixed top-4 right-4 z-[9999] w-96 max-w-sm bg-black/90 backdrop-blur-xl text-gray-100 rounded-xl shadow-[0_0_40px_rgba(41,121,255,0.7)] border border-blue-500/60 transition-all duration-500 ease-out transform overflow-hidden pointer-events-auto"
      style={{
        background: 'linear-gradient(135deg, rgba(18,24,33,0.98) 0%, rgba(41,121,255,0.25) 100%)',
        display: isOpen ? 'block' : 'none', // Force display based on isOpen
        boxShadow: '0 0 40px rgba(41,121,255,0.7), 0 0 20px rgba(41,121,255,0.5) inset',
          ...
          <div className="flex items-start pl-2 group transition-all duration-300 hover:translate-x-1">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2">
                {school.programTypes.slice(0, 6).map((prog, index) => (
                  <span
                    key={index}
          ...
          <div className="flex items-start pl-2 group transition-all duration-300 hover:translate-x-1">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-3">
                {school.programs.slice(0, 1).map((prog, index) => (
                    ...
                    {prog.specializations && prog.specializations.length > 0 && (
                      <div className="mt-2">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {prog.specializations.slice(0, 3).map((spec, i) => (
